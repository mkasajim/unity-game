...
namespace NiobiumStudios
{
    ...
    public class DailyRewardsInterface : MonoBehaviour
    {
        ...
        private void OnClaimPrize(int day)
        {
            ...
            GameManager.Instance.AddCoin(rewardQt);
            Debug.Log(rewardQt);
            bonusReward = rewardQt;
        ...
        {
            Debug.Log("ODUL ALINDI CARPI 2");          
            GameManager.Instance.AddCoin(bonusReward);
            Debug.Log(bonusReward);
            panelReward.SetActive(false);
            canvas.gameObject.SetActive(false);
          //  FindObjectOfType<MediationManager>().ShowAdmobRewardVideo(3);
